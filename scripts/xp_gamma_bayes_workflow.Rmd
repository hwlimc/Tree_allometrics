---
title: "Bayesian XP Gamma Model Diagnostics"
output:
  html_document:
    toc: true
    toc_depth: 2
...
if (!is.null(input_file) && nzchar(input_file)) {
  project_root <- normalizePath(file.path(dirname(input_file), ".."), mustWork = TRUE)
} else {
  wd <- normalizePath(getwd(), mustWork = TRUE)
  if (basename(wd) %in% c("scripts", "bayes_outputs")) {
    project_root <- normalizePath(file.path(wd, ".."), mustWork = TRUE)
  } else {
    project_root <- wd
  }
}
...
The manifest is the only table that stores model file names and model file timestamps. All other output tables use the `model` column as the key.
...
# Step 2: Posterior Parameter Estimates
...
This step compares observed values with posterior predictive draws (`yrep`). The response is reported on the original scale: `befa.st = y1m1 + 1` and `befr.st = y2`.

```{r step-3-ppcheck-data}
...
```{r step-3-ppcheck-observed-vs-yrep-plot}
pdf(file.path(out_dir, "03_ppcheck_observed_vs_yrep.pdf"), width = 12, height = 7)
...
for (resp in c("befa.st", "befr.st")) {
  ...
  for (mod in names(fits)) {
    d <- step3_yrep[step3_yrep$response == resp & step3_yrep$model == mod, ]
    lim <- range(c(d$observed, d$yrep_q05, d$yrep_q95), na.rm = TRUE)
    ...
    plot(
      ...
      col = rgb(0, 0, 0, 0.35),
      xlab = "Observed",
      ylab = "Posterior predictive median",
  ...
  mtext(paste("Observed vs posterior predictive yrep:", resp), outer = TRUE, cex = 1.2)
}
...
```{r step-4-observed-predicted-data}
...
```{r step-4-observed-predicted-plot}
